...
public class CuotaDao {
	...
	public Cuota traer(long idCliente) throws HibernateException {
		Cuota objeto = null;
		try {
			iniciaOperacion();
			objeto = (Cuota) session.get(Cuota.class, idCliente);
		} finally {
			session.close();
		}
		return objeto;
	...
	public List<Cuota> traer() throws HibernateException {
		...
		try {
			iniciaOperacion();
			lista = session.createQuery("from Cuota c order by nroCuota asc").list();
		} finally {
			session.close();
		}
		return lista;
